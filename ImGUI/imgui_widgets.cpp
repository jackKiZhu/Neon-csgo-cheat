...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool hflag)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
	...
	if (hovered && !hflag) {
		draw->AddRectFilled(bb.Min, bb.Max, ImColor(255, 0, 87, 150));
	}
	if (!hovered && !hflag) {
		draw->AddRectFilled(bb.Min, bb.Max, ImColor(100, 100, 100, 100));
	}
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, bool hflag)
{
    return ButtonEx(label, size_arg, 0, hflag);
}
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
	...
	if (ImGui::IsItemHovered()) {
		window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(255, 255, 255, 150));
	}
}
...
bool ImGui::ImageButton(const char* label, ImTextureID user_texture_id, ImTextureID user_texture_id_hovered, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
	...
	if (hovered) {
		window->DrawList->AddImage(user_texture_id_hovered, image_bb.Min, image_bb.Max, uv0, uv1, GetColorU32(tint_col));
	}
    else window->DrawList->AddImage(user_texture_id, image_bb.Min, image_bb.Max, uv0, uv1, GetColorU32(tint_col));
	//window->DrawList->AddText((image_bb.Min + image_bb.Max) / 2 - ImVec2(sizeof(label) / 1.2, sizeof(label) / 1.8) * 4, ImColor(255, 255, 255, 255), label);
	//window->DrawList->AddText({image_bb.Min.x + size.x / 2 - sizeof(label) * 4, image_bb.Min.y + 5}, ImColor(255, 255, 255, 255), label);
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    return pressed;
}
/*
bool ImGui::Checkbox(const char* label, bool* v) хуле ты мне тут выписываешь алло
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
	//RenderNavHighlight(total_bb, id);
	RenderFrame(check_bb.Min, check_bb.Max, ImColor(10, 31, 51), true, style.FrameRounding);
	RenderFrame(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), ImColor(9, 20, 37), true, style.FrameRounding);
	if (*v)
	{
		const float pad = ImMax(1.0f, (float)(int)(square_sz / 6.0f));
		ImVec2 size = check_bb.Max - check_bb.Min;
		window->DrawList->AddLine(ImVec2(check_bb.Min.x + 5, check_bb.Min.y - 1 + size.y / 2.f), ImVec2(check_bb.Min.x + size.x / 2.f, check_bb.Min.y - 5 + size.y), ImColor(255, 0, 87, 255), 3.f);
		window->DrawList->AddLine(ImVec2(check_bb.Min.x + size.x / 2.f, check_bb.Min.y - 5 + size.y), ImVec2(check_bb.Min.x + size.x - 4, check_bb.Min.y + 4), ImColor(255, 0, 87, 255), 3.f);
		window->DrawList->AddLine(ImVec2(check_bb.Min.x + size.x / 2.f, check_bb.Min.y - 5 + size.y), ImVec2(check_bb.Min.x + size.x / 2.f + 1.f, check_bb.Min.y - 5 + size.y), ImColor(255, 0, 87, 255), 3.f);
	...
	if (g.LogEnabled)
		LogRenderedText(&total_bb.Min, *v ? "[x]" : "[ ]");
	if (label_size.x > 0.0f)
		RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y - 3.f), label);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (hovered || selected) {
		draw->AddRectFilled(bb.Min, bb.Max, ImColor(225, 0, 87, 100));
	}
