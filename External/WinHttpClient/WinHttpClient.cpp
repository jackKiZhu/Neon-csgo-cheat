...
bool WinHttpClient::SendHttpRequest(const wstring &httpVerb)
{
	...
	hSession = ::WinHttpOpen(SZ_AGENT,
							 WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, // 8.1/newer == WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, no == WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY
	...
	if (::WinHttpCrackUrl(m_URL.c_str(), m_URL.size(), 0, &urlComp))
	{
		...
		if (hConnect != NULL)
		{
			...
			DWORD dwOpenRequestFlag = (urlComp.nScheme == 2) ? WINHTTP_FLAG_SECURE : 0;
