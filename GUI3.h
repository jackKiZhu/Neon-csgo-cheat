			...
			case 1:
			{
				...
				if (CurrentWeapon > 0) {
					...
					*/
					...
					//	s(); ImGui::SliderInt(language ? u8"End silent bullet" : u8"Пуля конца", &g_Weapons[CurrentWeapon].pSilentBullet, 0, 30);
	...
	ImGui::BeginChild("##MainChildWindows", ImVec2(547, 543), false, dwFlag);
	{
...

		{
			...
			if (globalstatus == 0) {
				...
				ImGui::Checkbox(language ? "Enable Changer" : u8"Включить скины", &Options::SkinChanger::EnabledSkin);
				ImGui::Checkbox(language ? "Enable Glove Changer" : u8"Включить перчатки", &Options::SkinChanger::EnabledGlove);
				ImGui::Checkbox(language ? "Enable Knife Changer" : u8"Включить ножи", &Options::SkinChanger::EnabledKnife);
				ImGui::Checkbox(language ? "Enable Stickers" : u8"Включить стикеры", &g_Weapons[CurrentWeapon].StickersEnabled);
				ImGui::Checkbox(language ? "Fast changer" : u8"Быстрый ченжер", &fastfrimen);
				ImGui::Checkbox(language ? "Safety changer" : u8"Безопасный ченжер", &safetychanger);
				if (safetychanger) {
				...
				//if (ImGui::Button(u8"Оружия")) globalstatus = 1;
				if (percent == 49) {
					...
					ImGui::SameLine(300);
					if (ImGui::ImageButton("", getTexture(49, Options::SkinChanger::Glovemenu), getTexture(49, Options::SkinChanger::Glovemenu), ImVec2(92, 69))) {
						globalstatus = 3;
					...
					//draw4->AddRectFilled(ImVec2(0, 400), ImVec2(500, 430), ImColor(255, 255, 255, 255));
					draw4->AddRectFilled(ImVec2(300, 350), ImVec2(300 + percent * 4, 380), ImColor(255, 255, 255, 255));
				}
			}
			if (globalstatus == 1) {
				if (safetychanger) {
			...
			if (globalstatus == 2) {
				...
				ImGui::SliderInt("ct", &g_Weapons[CurrentWeapon].ChangerSkinCT, 0, 1000); //кстати  утебя неправильно поставлены некоторые ножи когда ставишь фальшион ставится боуи
				*/
			//	ImGui::Text(std::to_string(g_Weapons[gun_definitioni[status]].ChangerSkinTT).c_str());
			...
			if (globalstatus == 3) {
				...
				for (int i = 0; i < 49 && status != 0; ++i) {
					...
					if (49 > i) {
						...
						if (ImGui::ImageButton("", getTexture(status, i), getTexture(status, i), ImVec2(92, 69))) {
							Options::SkinChanger::Glovemenu = i;
							
							gunSkins[status] = i;
							
							Options::SkinChanger::GLoveIdx = glove_def[i];
			...
			std::string skinName = U::GetWeaponNameById(CurrentWeapon);
			...
			if ((g_pEngine->IsConnected() || g_pEngine->IsInGame() ) && isdownloaded)
			{
				...
				ImGui::InputInt((language ? "Stat track" : u8"Статтрек"), &g_Weapons[CurrentWeapon].ChangerStatTrakTT);
				ImGui::Checkbox(language ? "Auto StatTrack" : u8"Авто статтрек", & g_Weapons[CurrentWeapon].enable_auto_stattrak);

				ImGui::SliderInt((language ? "Seed" : u8"Сид"), &g_Weapons[CurrentWeapon].ChangerSeedTT, 0, 100);
				ImGui::SliderFloat((language ? "Wear" : u8"Поношенность"), &g_Weapons[CurrentWeapon].ChangerWearTT, 0, 1);

				int unk_xuina = 0;
				ImGui::Spacing();
				//ImGui::Combo("##SkinGLOVE", &Options::SkinChanger::GLoveIdx, gloves_listbox_items, ARRAYSIZE(gloves_listbox_items));
				ImGui::Text(language ? "Stickers" : u8"Стикеры");
				ImGui::SliderInt((language ? "Slot" : u8"Слот"), &selected_sticke, 1, 4);
				ImGui::SliderFloat((language ? "Scale" : u8"Размер"), &g_Weapons[CurrentWeapon].Stickers[selected_sticke].flScale, 0.f, 1.f);
				...
				ImGui::SliderFloat((language ? "Sticker wear" : u8"Поношенность стикера"), &g_Weapons[CurrentWeapon].Stickers[selected_sticke].flWear, 0.f, 1.f);

				ImGui::InputInt((u8"Id"), &g_Weapons[CurrentWeapon].Stickers[selected_sticke].iID);
	...
	ImGui::BeginChild("##MainChildWindows", ImVec2(547, 543), false, dwFlag);
	{
...

		{
			if (CurrentWeapon > 0) {
				...
				ImGui::Spacing();
				ImGui::Text(u8"Возьми оружие в руки уеба");
			}
...
namespace CMenu
{
	...
	void color( ) {
		...
		style.Colors[ImGuiCol_MenuBarBg] = ImVec4(1.00f, 0.47f, 0.47f, 1.00f);
		style.Colors[ImGuiCol_ScrollbarBg] = ImVec4(0.04f, 1.00f, 0.15f, 1.00f);
		...
		style.Colors[ImGuiCol_CheckMark] = ImVec4(0.44f, 1.00f, 0.81f, 1.00f);
		style.Colors[ImGuiCol_SliderGrab] = ImVec4(0.33f, 0.78f, 0.78f, 0.60f);
		style.Colors[ImGuiCol_SliderGrabActive] = ImVec4(1.00f, 0.78f, 1.00f, 1.00f);
		style.Colors[ImGuiCol_Button] = ImVec4(0.6f, 0.6f, 1.00f, 1.00f); //style.Colors[ImGuiCol_Button] = ImVec4(0.4f, 0.55f, 0.55f, 1.00f);
		style.Colors[ImGuiCol_ButtonHovered] = ImVec4(0.7f, 1.00f, 0.7f, 1.f);
		style.Colors[ImGuiCol_ButtonActive] = ImVec4(1.00f, 0.8f, 0.8f, 1.00f);
		style.Colors[ImGuiCol_Header] = ImVec4(0.15f, 1.00f, 1.00f, 0.00f);
		style.Colors[ImGuiCol_HeaderHovered] = ImVec4(1.00f, 0.78f, 0.78f, 0);
		style.Colors[ImGuiCol_HeaderActive] = ImVec4(1.00f, 0.78f, 1.00f, 0);
		style.Colors[ImGuiCol_Column] = ImVec4(0.25f, 0.74f, 1.00f, 1.00f);
		...
		style.Colors[ImGuiCol_ResizeGrip] = ImVec4(1.00f, 0.61f, 0.61f, 1.00f);
		style.Colors[ImGuiCol_ResizeGripHovered] = ImVec4(1.00f, 0.84f, 0.84f, 0.67f);
		style.Colors[ImGuiCol_ResizeGripActive] = ImVec4(0.53f, 1.00f, 1.00f, 1.00f);
		...
		style.Colors[ImGuiCol_PlotLines] = ImVec4(0.53f, 1.00f, 0.84f, 1.00f);
		...
		style.Colors[ImGuiCol_TextSelectedBg] = ImVec4(1.00f, 1.00f, 0.40f, 1.00f);
		...
		if (ImGui::Begin("##MainWindowz", &G::MenuOpened, ImVec2(900, 900), 0.f, dwFlag)) {
			{
				...
				draw->AddText(ImVec2(win_pos.x + 689, win_pos.y + 10), ImColor(255, 255, 255), (language == 0) ? " RU" : " US");
				//ImGui::PopFont();
		...
		ImGui::BeginChild("##Child", ImVec2(510, 476), false, dwFlag);
		{
			...
			case 1:
			{
				if (Options::Visuals::Misc::StreamMode) {
					...
					if (Options::Visuals::Misc::StreamMode) {
						s(); ImGui::Checkbox(u8"Рисовать прицел на снайперских винтовках", &Options::Misc::SniperCrosshairFalse);
					}
					...
					s(); ImGui::InputText(u8"##ClanTagAnimText", Options::Misc::ClanTagAnimText, 100);
					...
					s(); ImGui::InputText(u8"##NameSpamText", Options::Misc::NameSpamText, 100);
					...
					s(); ImGui::InputText(u8"##ChatSpamText", Options::Misc::ChatSpamText, 100);
					...
					s(); ImGui::Checkbox(language ? u8"Legit Desync" : u8"Легит Десинки", &Options::Misc::Desync);//Options::Visuals::Misc::FakeduckKey
